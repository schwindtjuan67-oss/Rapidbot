...
class OrderManager:
    ...
    def _place_entry(self) -> None:
        ...
        # SPOT mode: leverage/margin ignored
        pass
...
@dataclass
class BTConfig:
    fee_taker: float = 0.0            # SPOT: 0% taker
...
def run_backtest(
    ...
    print(f"total_fees_paid: {float(metrics.get('total_fee_paid_usd', 0.0))}")
    print(f"final equity: {float(metrics.get('final_equity', final_equity))}")
    print(f"max drawdown: {float(metrics.get('max_drawdown_pct', metrics.get('max_dd_pct', 0.0)))}")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--margin_mode", type=str, default=None)
    ap.add_argument("--reduce_only", type=int, default=None)
    args = ap.parse_args()
