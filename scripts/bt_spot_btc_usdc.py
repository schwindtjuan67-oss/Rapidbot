...
def _build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(description="Backtest SPOT BTC/USDC long-only using CSV 5m data")
    ...
    ap.add_argument("--symbol", type=str, default="BTC/USDC")
...
def _resolve_outdir(outdir: str) -> str:
    ...
    return os.path.join("results", "bt_spot_btc_usdc", stamp)
...
def main() -> None:
    ...
    if args.symbol != "BTC/USDC":
        raise SystemExit("Only --symbol BTC/USDC is supported by this entrypoint.")

    outdir = _resolve_outdir(args.outdir)
    os.makedirs(outdir, exist_ok=True)

    print(
        "BT SPOT BTC/USDC — fees: "
    ...
    # SPOT BTC/USDC entrypoint is long-only.
