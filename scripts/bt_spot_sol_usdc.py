...
def _build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(description="Backtest SPOT SOL/USDC long-only using CSV 5m data")
    ap.add_argument("--bt", action="store_true", help="Run backtest mode")
    ap.add_argument("--csv", type=str, required=True, help="Historical 5m CSV with ts,open,high,low,close,volume (expected: SOLUSDC_5m_fixed.csv)")
    ap.add_argument("--symbol", type=str, default="SOL/USDC")
...
def _resolve_outdir(outdir: str) -> str:
    ...
    return os.path.join("results", "bt_spot_sol_usdc", stamp)
...
def main() -> None:
    ...
    if args.symbol != "SOL/USDC":
        raise SystemExit("Only --symbol SOL/USDC is supported by this entrypoint.")

    outdir = _resolve_outdir(args.outdir)
    os.makedirs(outdir, exist_ok=True)

    print(
        "BT SPOT SOL/USDC — fees: "
    ...
    # SPOT SOL/USDC entrypoint is long-only.
